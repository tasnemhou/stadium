...
--用户信息表
  CREATE TABLE EMP_USER_INFO
   ...
   comment on column emp_pay_info.dealAmount is '交易金额';
   comment on column emp_pay_info.dealTimes is '交易次数';
   comment on column emp_pay_info.balance is '剩余金额';
   comment on column emp_pay_info.times is '剩余使用次数';
   comment on column emp_pay_info.kind is '交易类型 0-充值 1-消费 2-免费（优惠）';
   comment on column emp_pay_info.customerType is '客户类型  0-普通会员   1-散客';
